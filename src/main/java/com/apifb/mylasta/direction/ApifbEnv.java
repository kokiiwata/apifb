...
public interface ApifbEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/apifb */
    ...
    /** The key of the configuration. e.g. returnpath@apifb.com */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. apifb-support@annie.example.com */
    String MAIL_ADDRESS_SUPPORT = "mail.address.support";

    /** The key of the configuration. e.g. org.h2.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:h2:file:$classes(com.apifb.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/apifbdb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. apifbdb */
    ...
    /** The key of the configuration. e.g. localhost:8090/apifb */
     ...
     * The value is, e.g. /tmp/lastaflute/apifb <br>
     ...
     * The value is, e.g. returnpath@apifb.com <br>
     ...
     * The value is, e.g. apifb-support@annie.example.com <br>
     * comment: Mail Address for Apifb Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. jdbc:h2:file:$classes(com.apifb.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/apifbdb <br>
     ...
     * The value is, e.g. localhost:8090/apifb <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements ApifbEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(ApifbEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(ApifbEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(ApifbEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(ApifbEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(ApifbEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(ApifbEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(ApifbEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(ApifbEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(ApifbEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(ApifbEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(ApifbEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(ApifbEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(ApifbEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(ApifbEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(ApifbEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(ApifbEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(ApifbEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(ApifbEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(ApifbEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(ApifbEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(ApifbEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(ApifbEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(ApifbEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(ApifbEnv.SERVER_DOMAIN);
        }
    }
}
